...
The **inner loop** is reconstruction under an HLT1 latency budget: Graph Clustering, GarNet, UzzieNet, deep sets, the cellular automaton Graph Clustering replaced. Architecture is part of the searchable space because a vertex-less radiative photon is a harder calorimeter object than an inclusive cluster. Click Λb → Λγ and the menu lights the families that matter for that topology.
